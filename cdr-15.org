...
* ISSUES
  ...
  1. Inconsistency between return values of ~add-package-local-nickname~ and
     ...
     *CURRENTLY*:
     ...
     sbcl, ccl, ecl, acl, abcl: ~add-package-local-nickname~ always returns
     /designated package/.

     sbcl, ccl, ecl, acl: ~remove-package-local-nickname~ returns ~T~ on success
     and ~NIL~
  ...
  7. It is not clear whether functions ~package-local-nicknames~ and
     ...
     *CURRENTLY*:
     ...
     sbcl, acl, ccl, abcl, ecl: don't return duplicate entries from ~package-local-nicknames~.

     sbcl, acl: don't return duplicate entries from ~package-locally-nicknamed-by-list~.
